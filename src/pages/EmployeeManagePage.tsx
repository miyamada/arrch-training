...
export default function EmployeeManagePage() {
  ...
  return (
    <div style={{ padding: '24px 40px' }}>
      ...
      {showForm && (
        <div style={{
          background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '4px',
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '16px' }}>
              ...
              <label>
                <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>担当メンター</div>
                <select
                  value={form.mentor_id}
                  onChange={e => setForm(f => ({ ...f, mentor_id: e.target.value }))}
                  style={{ width: '100%', padding: '9px 12px', background: '#f7f8fa', border: '1px solid #e5e7eb', borderRadius: '4px', color: '#111827', fontSize: '13px' }}
                >
              ...
              <label>
                <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>ロール</div>
                <select
                  value={form.role}
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button type="submit" disabled={saving} style={{ padding: '9px 20px', background: saving ? '#f0f2f5' : '#c8a96a', color: saving ? '#9ca3af' : '#ffffff', border: 'none', borderRadius: '4px', fontSize: '12px', fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}>
                {saving ? '追加中...' : '社員を追加'}
              </button>
              <button type="button" onClick={() => setShowForm(false)} style={{ padding: '9px 20px', background: 'none', color: '#6b7280', border: '1px solid #e5e7eb', borderRadius: '4px', fontSize: '12px', cursor: 'pointer' }}>
                キャンセル
              </button>
            </div>
          </form>
      ...
      {editTarget && (
        <div style={{ background: '#ffffff', border: '1px solid #c8a96a', borderRadius: '4px', padding: '24px', marginBottom: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
            ...
            <button onClick={() => setEditTarget(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af' }}>
              <X size={16} />
            </button>
          </div>
          <form onSubmit={handleEdit}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '16px' }}>
              <label>
                <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>名前</div>
                <input type="text" required value={editForm.name} onChange={e => setEditForm(f => ({ ...f, name: e.target.value }))}
                  style={{ width: '100%', padding: '9px 12px', background: '#f7f8fa', border: '1px solid #e5e7eb', borderRadius: '4px', color: '#111827', fontSize: '13px' }} />
              </label>
              <label>
                <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>入社日</div>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button type="submit" disabled={editSaving} style={{ padding: '9px 20px', background: editSaving ? '#f0f2f5' : '#c8a96a', color: editSaving ? '#9ca3af' : '#ffffff', border: 'none', borderRadius: '4px', fontSize: '12px', fontWeight: 600, cursor: editSaving ? 'not-allowed' : 'pointer' }}>
                {editSaving ? '保存中...' : '保存'}
              </button>
              <button type="button" onClick={() => setEditTarget(null)} style={{ padding: '9px 20px', background: 'none', color: '#6b7280', border: '1px solid #e5e7eb', borderRadius: '4px', fontSize: '12px', cursor: 'pointer' }}>
                キャンセル
              </button>
            </div>
          </form>
      ...
      <section style={{ marginBottom: '32px' }}>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '4px', overflow: 'hidden' }}>
          {trainees.length === 0 ? (
            <div style={{ padding: '24px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>研修中の社員はいません</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #e5e7eb', background: '#f7f8fa' }}>
                  {['名前', 'メール', '入社日', 'メンター', ''].map(h => (
                    <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: '11px', color: '#9ca3af', fontWeight: 600 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {trainees.map((emp, i) => {
                  ...
                  return (
                    <tr key={emp.id} style={{ borderBottom: i < trainees.length - 1 ? '1px solid #f0f2f5' : 'none' }}>
                      <td style={{ padding: '12px 16px', color: '#111827' }}>{emp.name}</td>
                      <td style={{ padding: '12px 16px', color: '#6b7280' }}>{emp.email}</td>
                      <td style={{ padding: '12px 16px', color: '#6b7280' }}>{format(parseISO(emp.joined_at), 'yyyy/MM/dd')}</td>
                      <td style={{ padding: '12px 16px', color: '#c8a96a' }}>{m?.name ?? '—'}</td>
                      <td style={{ padding: '12px 16px' }}>
                        <button onClick={() => openEdit(emp)} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '5px 10px', background: 'none', border: '1px solid #e5e7eb', borderRadius: '4px', color: '#6b7280', cursor: 'pointer', fontSize: '11px' }}>
                          <Pencil size={11} /> 編集
                        </button>
                      </td>
                    </tr>
      ...
      <section>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '4px', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            ...
            <tbody>
              {admins.map((emp, i) => (
                <tr key={emp.id} style={{ borderBottom: i < admins.length - 1 ? '1px solid #f0f2f5' : 'none' }}>
                  <td style={{ padding: '12px 16px', color: '#111827' }}>{emp.name}</td>
                  <td style={{ padding: '12px 16px', color: '#6b7280' }}>{emp.email}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <button onClick={() => openEdit(emp)} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '5px 10px', background: 'none', border: '1px solid #e5e7eb', borderRadius: '4px', color: '#6b7280', cursor: 'pointer', fontSize: '11px' }}>
