      ...
      {editing && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}>
            ...
            <button onClick={() => setEditing(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b7280' }}>
              <X size={16} />
            </button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            <label>
              <div style={{ fontSize: '11px', color: '#4b5563', marginBottom: '6px' }}>フェーズ</div>
              <select
                value={editing.phase}
                onChange={e => setEditing(ed => ({ ...ed, phase: Number(e.target.value) }))}
                style={{ width: '100%', padding: '8px 10px', background: '#eef0f3', border: '1px solid #d1d5db', borderRadius: '4px', color: '#111827', fontSize: '13px' }}
              >
                {[1, 2, 3, 4].map(p => <option key={p} value={p}>フェーズ{p}「{PHASE_NAMES[p]}」</option>)}
              </select>
            </label>
            <label>
              <div style={{ fontSize: '11px', color: '#4b5563', marginBottom: '6px' }}>担当種別</div>
              <select
            ...
            <label>
              <div style={{ fontSize: '11px', color: '#4b5563', marginBottom: '6px' }}>カテゴリ</div>
              <input
                type="text"
            ...
            <label>
              <div style={{ fontSize: '11px', color: '#4b5563', marginBottom: '6px' }}>並び順</div>
              <input
                type="number"
          ...
          <label style={{ display: 'block', marginBottom: '12px' }}>
            <div style={{ fontSize: '11px', color: '#4b5563', marginBottom: '6px' }}>内容</div>
            <input
              type="text"
          ...
          <label style={{ display: 'block', marginBottom: '12px' }}>
            <div style={{ fontSize: '11px', color: '#4b5563', marginBottom: '6px' }}>完了定義</div>
            <input
              type="text"
          ...
          <label style={{ display: 'block', marginBottom: '16px' }}>
            <div style={{ fontSize: '11px', color: '#4b5563', marginBottom: '6px' }}>動画URL</div>
            <input
              type="url"
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '20px', cursor: 'pointer' }}>
            ...
            />
            <span style={{ fontSize: '12px', color: '#4b5563' }}>★ 社内検定（合格必須）</span>
          </label>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button
              ...
              style={{
                display: 'flex', alignItems: 'center', gap: '6px',
                padding: '9px 20px', background: saving ? '#d1d5db' : '#c8a96a',
                color: saving ? '#6b7280' : '#ffffff', border: 'none', borderRadius: '4px',
            ...
            <button
              ...
              style={{ padding: '9px 20px', background: 'none', color: '#4b5563', border: '1px solid #d1d5db', borderRadius: '4px', fontSize: '12px', cursor: 'pointer' }}
            >
              キャンセル
            </button>
          </div>
            ...
            {phItems.map((item, i) => (
              <div
                ...
                style={{
                  ...
                  gap: '12px',
                  padding: '12px 16px',
                  borderBottom: i < phItems.length - 1 ? '1px solid #d1d5db' : 'none',
                }}
              >
                <span style={{ fontSize: '11px', color: '#6b7280', minWidth: '24px' }}>{item.sort_order}</span>
                <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button
                    ...
                    style={{ padding: '6px 10px', background: 'none', border: '1px solid #d1d5db', borderRadius: '4px', color: '#4b5563', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px' }}
                  >
                    <Pencil size={11} /> 編集
                  </button>
                  <button
